...
class ConfigManager():
	...
	def can_set_value(self, obj:discord.Object, name, val):
		...
		if self.type == "guild" and obj.owner.id == self.client.aidan:
		...
		def single_check(val):
			...
			if self.get_type(name) == "number":
				if len(str(val)) > number_limit:
					return "Number values like `{name}` can't have more than {limit} digits.".format(limit=number_limit, name=name)
			opts = self.get_options(name)
			if opts and val not in opts:
				return "The value `{name}` has options, only use values from the list below.\n`{opts}`".format(opts=", ".join(opts), name=name)
			return False
